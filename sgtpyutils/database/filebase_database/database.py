...
class Database(ISaver):
    ...
    async def save_async_chunked(
        self, chunk_size: int = 1024 * 1024, sleep_ms: int = 1
    ) -> bool:
        ...
        - 每次写入后 yield 事件循环 sleep_ms 毫秒，防止阻塞
        - 适合已知的大文件（如 1GB+ 的单个 database）

        Args:
            chunk_size: 每次写入的字节数（默认 1MB，建议 64KB~4MB）
            sleep_ms: 每次写入后 sleep 的毫秒数（默认 1ms，0 = 仅 yield）
        """
        if self._is_deleted:
            return False
        if self._database not in Database.cache:
            return False
        data = self._data_obj.to_dict()
        return await save_direct_async_chunked(
            self.database_filename, data, chunk_size, sleep_ms
        )
    ...
    @classmethod
    async def save_all_async_chunked(
        cls, chunk_size: int = 1024 * 1024, sleep_ms: int = 1
    ) -> tuple[int, int]:
        ...
        tasks = [
            save_direct_async_chunked(
                cls._build_filename(name), data_obj.to_dict(), chunk_size, sleep_ms
            )
            for name, data_obj in list(cls.cache.items())
        ]
        results = await asyncio.gather(*tasks, return_exceptions=True)
